...
export function MatchesCarousel({ matches, retakeHref, dashboardHref }: MatchesCarouselProps) {
  ...
  const { cardWidths, offsets } = useMemo(() => {
    const widths = matches.map(() => (isDesktop ? VW_CARD_DESKTOP : MOBILE_CARD_W));
  ...
  return (
    <div className="relative h-screen w-screen flex flex-col overflow-hidden bg-[var(--background)]">
      ...
      <div className="pt-6 pb-2 text-center z-10">
        <h1 className="page-title">Your creative spark is calling</h1>
            ...
            return (
              ...
              >
                <div className={`h-full w-full ${isActive ? "" : "pointer-events-none"}`}>
                  <MatchCard match={match} index={idx} theme={CARD_THEMES[idx % 2]} />
                </div>
              </div>
            );
          })}
      ...
      <div className="flex justify-center items-center flex-shrink-0 z-10 pb-6 text-[var(--foreground)]/60">
        <p className="text-sm">
          ...
          <Link href={retakeHref} className="hover:text-[var(--primary)] font-medium">
            <em>retake quiz</em>
          </Link>
          {" "}or{" "}
          <Link href={dashboardHref} className="hover:text-[var(--primary)] font-medium">
