...
import { Home, BarChart3, User, Settings, Compass, Menu, LogOut, ChevronsLeft, ChevronsRight } from "lucide-react";
import { useUser } from "@/lib/hooks/useUser";

const navItems = [
  { href: "/dashboard", label: "My Hobbies", Icon: Home },
  { href: "/dashboard/progress", label: "Progress", Icon: BarChart3 },
  { href: "/discover", label: "Discover", Icon: Compass },
];

const mobileNavItems = [
  { href: "/dashboard", label: "Hobbies", Icon: Home },
  { href: "/dashboard/progress", label: "Progress", Icon: BarChart3 },
  { href: "/discover", label: "Discover", Icon: Compass },
];
...
export function DashboardNav({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex">
      ...
      {/* ── Content ── */}
      <main
        className={`flex-1 min-h-screen pt-20 pb-20 md:pt-6 md:pb-6 p-4 md:p-6 transition-all duration-500 ease-[bezier(0.25,1,0.5,1)] ${desktopOpen ? "md:pl-80" : "md:pl-32"
          }`}
      >
        {children}
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-30 flex items-center justify-around h-16 shadow-sm">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-1 py-2 px-6 transition-colors ${
                active ? "text-[var(--secondary)]" : "text-gray-400 hover:text-gray-600"
              }`}
            >
